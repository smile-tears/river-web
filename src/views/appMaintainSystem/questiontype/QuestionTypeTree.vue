...
export default {
  created () {
    this.getTree('init')
    this.eventBus.$on('refreshQuestionTypeTree', questionTypeNode => {
      // 接收事件
  ...
  methods: {
        ...
        .then(res => {
          if (res.code === 200) {
            ...
            if (type === 'init') {
              ...
              this.eventBus.$emit('transferQuestionTypeNode', res.result[0])
            }
          }
        })
        .catch(() => {
    ...
    onSelect (selectedKeys, info) {
      this.eventBus.$emit('transferQuestionTypeNode', info.node)
    },
    onChange (e) {
